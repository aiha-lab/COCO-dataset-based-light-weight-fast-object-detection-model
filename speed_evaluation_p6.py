...
from yolox.models import YOLOPAFPNCustomP6, YOLOXCustomP6, YOLOXHeadCustom
from yolox.utils.model_utils import fuse_model
from speed_evaluation import run_module


def build_yolox_p6():
    # M config
    backbone = YOLOPAFPNCustomP6(depth=0.67, width=0.75, act="hard_swish", in_channels=(256, 512, 768, 1024))
    head = YOLOXHeadCustom(num_classes=80, width=0.75, act="hard_swish", in_channels=(256, 512, 768, 1024))
    net = YOLOXCustomP6(backbone, head)
    return net


def total_latency_p6(net, use_half: bool = False, input_size: int = 640):
    net: YOLOXCustomP6
    if use_half:
    ...
    print(f"Dark5-CSP time: {dark5_out1_time:.3f} us "
          f"(out shape: {tuple(dark5_out1.shape)}) "
          f"(count: {dark5_out1_count})")
    x1 = dark5_out1

    dark6_out0, dark6_out0_time, dark6_out0_count = run_module(net.backbone.backbone.dark6[0], dark5_out1)
    print(f"Dark6-Conv time: {dark4_out0_time:.3f} us "
          f"(out shape: {tuple(dark6_out0.shape)}) "
          f"(count: {dark6_out0_count})")
    dark6_out1, dark6_out1_time, dark6_out1_count = run_module(net.backbone.backbone.dark6[1], dark6_out0)
    print(f"Dark6-SPP time: {dark6_out1_time:.3f} us "
          f"(out shape: {tuple(dark6_out1.shape)}) "
          f"(count: {dark6_out1_count})")
    dark6_out2, dark6_out2_time, dark6_out2_count = run_module(net.backbone.backbone.dark6[2], dark6_out1)
    print(f"Dark6-CSP time: {dark6_out2_time:.3f} us "
          f"(out shape: {tuple(dark6_out2.shape)}) "
          f"(count: {dark6_out2_count})")
    x0 = dark6_out2
    ...
    print(f"FPN-lateral0-upsample time: {f_out0_0_time + f_out0_1_time:.3f} us "
          f"(out shape: {tuple(f_out0.shape)}) "
          f"(count: {f_out0_0_count + f_out0_1_count})")
    f_out0 = torch.cat([f_out0, x1], dim=1)
    f_out0, f_out0_2_time, f_out0_2_count = run_module(net.backbone.C3_p5, f_out0)
    print(f"FPN-C3-p5 time: {f_out0_2_time:.3f} us "
          f"(out shape: {tuple(f_out0.shape)}) "
          f"(count: {f_out0_2_count})")

    fpn_out1, f_out1_0_time, f_out1_0_count = run_module(net.backbone.lateral_conv1, f_out0)
    f_out1, f_out1_1_time, f_out1_1_count = run_module(net.backbone.upsample, fpn_out1)
    print(f"FPN-lateral1-upsample time: {f_out1_0_time + f_out1_1_time:.3f} us "
          f"(out shape: {tuple(f_out1.shape)}) "
          f"(count: {f_out1_0_count + f_out1_1_count})")
    f_out1 = torch.cat([f_out1, x2], dim=1)
    f_out1, f_out1_2_time, f_out1_2_count = run_module(net.backbone.C3_p4, f_out1)
    print(f"FPN-C3-p4 time: {f_out1_2_time:.3f} us "
          f"(out shape: {tuple(f_out1.shape)}) "
          f"(count: {f_out1_2_count})")

    fpn_out2, f_out2_0_time, f_out2_0_count = run_module(net.backbone.reduce_conv1, f_out1)
    f_out2, f_out2_1_time, f_out2_1_count = run_module(net.backbone.upsample, fpn_out2)
    print(f"FPN-reduce-upsample time: {f_out2_0_time + f_out2_1_time:.3f} us "
          f"(out shape: {tuple(f_out2.shape)}) "
          f"(count: {f_out2_0_count + f_out2_1_count})")
    f_out2 = torch.cat([f_out2, x3], dim=1)
    pan_out3, f_out2_2_time, f_out2_2_count = run_module(net.backbone.C3_p3, f_out2)
    print(f"FPN-C3-p3 time: {f_out2_2_time:.3f} us "
          f"(out shape: {tuple(pan_out3.shape)}) "
          f"(count: {f_out2_2_count})")

    p_out2, p_out2_0_time, p_out2_0_count = run_module(net.backbone.bu_conv2, pan_out3)
    print(f"FPN-bu-conv2 time: {p_out2_0_time:.3f} us "
          f"(out shape: {tuple(p_out2.shape)}) "
          f"(count: {p_out2_0_count})")
    p_out2 = torch.cat([p_out2, fpn_out2], dim=1)
    pan_out2, p_out2_1_time, p_out2_1_count = run_module(net.backbone.C3_n3, p_out2)
    print(f"FPN-C3-n3 time: {p_out2_1_time:.3f} us "
          f"(out shape: {tuple(pan_out2.shape)}) "
          f"(count: {p_out2_1_count})")

    p_out1, p_out1_0_time, p_out1_0_count = run_module(net.backbone.bu_conv1, pan_out2)
    print(f"FPN-bu-conv1 time: {p_out1_0_time:.3f} us "
          f"(out shape: {tuple(p_out1.shape)}) "
          f"(count: {p_out1_0_count})")
    p_out1 = torch.cat([p_out1, fpn_out1], dim=1)
    pan_out1, p_out1_1_time, p_out1_1_count = run_module(net.backbone.C3_n4, p_out1)
    print(f"FPN-C3-n4 time: {p_out1_1_time:.3f} us "
          f"(out shape: {tuple(pan_out1.shape)}) "
          f"(count: {p_out1_1_count})")

    p_out0, p_out0_0_time, p_out0_0_count = run_module(net.backbone.bu_conv0, pan_out1)
    print(f"FPN-bu-conv0 time: {p_out0_0_time:.3f} us "
          f"(out shape: {tuple(p_out0.shape)}) "
          f"(count: {p_out0_0_count})")
    p_out0 = torch.cat([p_out0, fpn_out0], dim=1)
    pan_out0, p_out0_1_time, p_out0_1_count = run_module(net.backbone.C3_n5, p_out0)
    print(f"FPN-C3-n5 time: {p_out0_1_time:.3f} us "
          f"(out shape: {tuple(pan_out0.shape)}) "
          f"(count: {p_out0_1_count})")

    head_out, head_time, head_count = run_module(net.head, (pan_out3, pan_out2, pan_out1, pan_out0))
    ...
    total_time = (
            ...
            dark5_out0_time + dark5_out1_time +
            dark6_out0_time + dark6_out1_time + dark6_out2_time +
            f_out0_0_time + f_out0_1_time + f_out0_2_time +
            f_out1_0_time + f_out1_1_time + f_out1_2_time +
            f_out2_0_time + f_out2_1_time + f_out2_2_time +
            p_out2_0_time + p_out2_1_time +
    ...
    total_count = (
            ...
            dark5_out0_count + dark5_out1_count +
            dark6_out0_count + dark6_out1_count + dark6_out2_count +
            f_out0_0_count + f_out0_1_count + f_out0_2_count +
            f_out1_0_count + f_out1_1_count + f_out1_2_count +
            f_out2_0_count + f_out2_1_count + f_out2_2_count +
            p_out2_0_count + p_out2_1_count +
...
if __name__ == '__main__':
    ...
    print(f"Model:\n{yolo}")
    total_latency_p6(yolo, use_half=True, input_size=768)
