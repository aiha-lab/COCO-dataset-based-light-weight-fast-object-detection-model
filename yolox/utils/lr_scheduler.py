...
class LRScheduler:
    def __init__(self, name: str, lr: float,
                 iters_per_epoch: int, total_epochs: int, **kwargs):
        """
...
def yolox_warm_cos_lr(lr,
    ...
    if iters <= warmup_total_iters:
        # lr = (lr - warmup_lr_start) * iters / float(warmup_total_iters) + warmup_lr_start  # x
        lr = (lr - warmup_lr_start) * pow(iters / float(warmup_total_iters), 2) + warmup_lr_start  # x^2
...
def yolox_semi_warm_cos_lr(lr,
    ...
    if iters <= warmup_total_iters:
        ...
        lr = min_lr + 0.5 * (lr - min_lr) * (1.0 + math.cos(
            math.pi * (normal_iters - warmup_total_iters + (iters - normal_iters) * iters_per_epoch
                       * 1.0 / iters_per_epoch_semi)
...
#     }
#
#     for i in range(kwargs["total_iters"]):
#         lr = yolox_warm_cos_lr(**kwargs, iters=i)
#         print(i, lr)
