...
from .dist import _get_global_gloo_group, get_world_size  # noqa
...
def get_async_norm_states(module) -> OrderedDict:
...
def pyobj2tensor(pyobj, device="cuda"):
    """serialize pickle-able python object to tensor"""
    storage = torch.ByteStorage.from_buffer(pickle.dumps(pyobj))  # noqa
    return torch.ByteTensor(storage).to(device=device)


def tensor2pyobj(tensor):
    """deserialize tensor to pickle-able python object"""
...
def all_reduce(py_dict, op="sum", group=None) -> dict:
    """
    ...
    Args:
        ...
        group (optional): distributed group (default: None)
    """
