...
class Pruner:
    def __init__(self, exp: MyExp, args):
        ...
        self.use_model_ema = False  # EMA should be false during pruning.
        ...
        self.val_loader = None  # for pruning
        ...
        self.max_val_iter = -1  # eval epoch iterations (works with train dataset)
    ...
    def train_one_iter(self):
        ...
        if self.iter % self.num_accumulate == (self.num_accumulate - 1):
            if self.epoch >= self.exp.prune_start_epoch:
    ...
    def before_train(self):
        ...
        # self.max_val_iter = (self.exp.prune_samples // self.args.batch_size) + 1
    ...
    def before_epoch(self):
        ...
        if self.exp.prune_start_epoch - 1 <= self.epoch:
            if self._first_prune:
                self.prune(threshold=0.03, verbose=True)
    ...
    def after_iter(self):
        ...
        if (self.epoch >= self.exp.prune_start_epoch) and (
                (self.iter + 1) % int(self.max_iter * self.exp.prune_interval) == 0):
            self.evaluate_and_save_model()
            self.prune()
    ...
    def prune(self, threshold=None, verbose: bool = False):
        logger.info("Pruning start at epoch: {}/{}, iter: {}/{}".format(
            self.epoch + 1, self.max_epoch, self.iter + 1, self.max_iter))

        all_reduce_norm(self.model)
        self.model.train()  # need to calculate loss
        ...
        candidates = {}
        for i, (images, targets, _, _) in enumerate(self.val_loader):
            images = images.to(torch.float32, non_blocking=True)
            targets = targets.to(torch.float32, non_blocking=True)
            targets.requires_grad = False
            images, targets = self.exp.preprocess(images, targets, self.exp.test_size)
            ...
            synchronize()
            if (i + 1) % 10 == 0:
                logger.info(f"... pruning iter {i + 1} / {self.max_val_iter}, total_loss: {loss.item():.3f}")

            for m_name, m in self.model.named_modules():
                if isinstance(m, BaseConv) and m.apply_prune:
                    if m_name not in candidates:
                        candidates[m_name] = m.score()
                    else:
                        candidates[m_name] += m.score()

        for k, v in candidates.items():
            candidates[k] = v * (1e4 / self.max_val_iter)  # average

        sorted_candidates = []
        for c in candidates.values():
            sorted_candidates.extend(c.tolist())
        sorted_candidates = sorted(sorted_candidates)
        sorted_candidates = [s for s in sorted_candidates if s < 1e6]  # exclude "inf"

        # TODO is this really synchronized through GPUs?
        if verbose:
            candidates_np = np.array(sorted_candidates)
            candidates_hist, candidates_edges = np.histogram(candidates_np, bins=256, density=False)
            s = f"... (index) hist | edges\n"
            for i in range(256):
                s += f"... ({i}) {candidates_hist[i]} | {candidates_edges[i + 1]:.3f}\n"
            logger.info(s)

        logger.info(f"... pruning total #channels in BaseConv (before prune): {len(sorted_candidates)}")
        if len(sorted_candidates) > self.exp.prune_channels:
            if threshold is None:
                threshold = sorted_candidates[self.exp.prune_channels]
            logger.info(f"...pruning threshold: {threshold:.6f} "
                        f"(min: {min(sorted_candidates):.6f}, max: {max(sorted_candidates):.6f})")

            # prune model
            for m_name, m in self.model.named_modules():
                if isinstance(m, BaseConv) and m.apply_prune:
                    m_mask = torch.greater_equal(candidates[m_name], threshold)
                    m.prune(m_mask)

        # manual print (all, remain)
        stat_keys = ("dark2", "dark3", "dark4", "dark5", "C3_p4", "C3_p3", "C3_n3", "C3_n4")
        stat = {k: [0, 0] for k in stat_keys}
        for m_name, m in self.model.named_modules():
            if isinstance(m, BaseConv) and m.apply_prune:
                for k in stat_keys:
                    if k in m_name:
                        stat[k][0] += len(m.mask.scale)
                        stat[k][1] += int(torch.sum(m.mask.scale).item())
        stat_str = "pruning remaining channels"
        for k in stat_keys:
            stat_str += f"\n... ({k}) {stat[k][0]} -> {stat[k][1]} (pruned {stat[k][0] - stat[k][1]})"
        total0 = sum([k[0] for k in stat.values()])
        total1 = sum([k[1] for k in stat.values()])
        stat_str += f"\n... (total) {total0} -> {total1} (pruned {total0 - total1})"
