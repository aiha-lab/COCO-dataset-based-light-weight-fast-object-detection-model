...
class CustomExp(BaseExp):
    ...
    def get_model(self, bn_momentum: float = 0.03):
        ...
        if getattr(self, "model", None) is None:
            ...
            backbone = YOLOPAFPNCustom(self.depth, self.width, in_channels=in_channels, act=self.act)
            head = YOLOXHeadCustom(self.num_classes, self.width, in_channels=in_channels, act=self.act)
            self.model = YOLOXCustom(backbone, head)
    ...
    def random_resize(self, data_loader, epoch, rank, is_distributed):
        ...
        if rank == 0:
            ...
            if self.random_size is None:
                min_size = int(self.input_size[0] / 32) + m_range[0]
                max_size = int(self.input_size[0] / 32) + m_range[1]
