...
class YOLOXHeadCustom(nn.Module):
    def __init__(self, num_classes,
        ...
        self.decode_in_inference = True  # for deploy, set to False  # TODO find out what this is
    ...
    def forward(self, xin, labels=None, images=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                if self.use_l1:
                    ...
                    h_size, w_size = reg_output.shape[-2:]
                    reg_output = reg_output.view(batch_size, self.n_anchors, 4, h_size, w_size).contiguous()
        ...
        if self.training:
            ...
            self.hw = [x.shape[-2:] for x in outputs]  # keep track of size
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        output = output.permute(0, 1, 3, 4, 2).reshape(batch_size, self.n_anchors * h_size * w_size, -1)
    ...
    def decode_outputs(self, outputs, dtype):
        grids = []
        strides = []
        for (h_size, w_size), stride in zip(self.hw, self.strides):
            yv, xv = torch.meshgrid([torch.arange(h_size), torch.arange(w_size)])
    ...
    def get_losses(
        ...
        loss_obj = (self.bce_loss(obj_preds.view(-1, 1), obj_targets)).sum() / num_fg
        loss_cls = (self.bce_loss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum() / num_fg
        if self.use_l1:
