...
class DistillLoss(nn.Module):
    ...
    def __init__(self, channels: int, teacher_channels=None):
        ...
        self.at_spatial_adapter = nn.Conv2d(1, 1, kernel_size=(3, 3), padding=1, bias=True)
        self.at_channel_adapter = nn.Linear(channels, teacher_channels, bias=True)
        self.am_adapter = nn.Conv2d(channels, teacher_channels, kernel_size=(1, 1), bias=True)

        self.l2_loss = nn.MSELoss()
        # TODO add non-local module
    ...
    def forward(self, student: torch.Tensor, teacher: torch.Tensor):
        ...
        at_spatial = self.at_spatial_adapter(at_spatial)  # (b, 1, h, w)
        ...
        at_channel = self.at_channel_adapter(at_channel)  # (b, c)
        ...
        am_loss = torch.sum(torch.square(self.am_adapter(student) - teacher)
...
class YOLODistiller(nn.Module):
    ...
    def __init__(self, width=1.0, in_channels=(256, 512, 1024)):
        ...
        self.dark3 = DistillLoss(channels=int(in_channels[0] * width))
        self.dark4 = DistillLoss(channels=int(in_channels[1] * width))
        self.dark5 = DistillLoss(channels=int(in_channels[2] * width))

        self.C3_p4 = DistillLoss(channels=int(in_channels[1] * width))
        self.C3_p3 = DistillLoss(channels=int(in_channels[0] * width))
        self.C3_n3 = DistillLoss(channels=int(in_channels[1] * width))
        self.C3_n4 = DistillLoss(channels=int(in_channels[2] * width))
