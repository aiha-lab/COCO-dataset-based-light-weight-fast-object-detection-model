...
def yolov5_load_one_image_pil(img_size: int, data_dir: str, img_file: str):
...
def yolov5_collate_batch(img_size: int, batch):
    ...
    for i, img in enumerate(img_list):
        ...
        pad_w = (max_w - w) // 2
        pad_h = (max_h - h) // 2
        # batch[i, :h, :w, :] = np.asarray(img)  # fit to top-left, preserve RGB
        batch[i, pad_h:pad_h + h, pad_w:pad_w + w, :] = np.asarray(img)  # fit to top-left, preserve RGB
