...
from yolox_infer.dali_dataset import YOLOXDaliImageFolderPipeline, YOLOXDaliIterator
...
def build_dali(cfg):
    pipeline = YOLOXDaliImageFolderPipeline(
        cfg["data_dir"], cfg["img_size"],
        batch_size=cfg["dataloader"]["batch_size"],
        queue_depth=cfg["dataloader"]["queue_depth"],
        num_threads=cfg["dataloader"]["num_threads"],
    )
    iterator = YOLOXDaliIterator(pipeline)
    print(f"DALI Dataloader initialized.")
...
def run(cfg, output_path: str, profile: bool, challenge: bool):
    ...
    for batch_i, (img, img_info) in enumerate(dataloader):
        ...
        # INPUT AUGMENTATION
        # img.sub_(114.0).mul_(0.9).add_(114.0)
        # y = 0.9 * (x - 114) + 114 = 0.9 * x + 11.4
